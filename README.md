...
- **🚀 高并发异步落库 (RabbitMQ)**
  为了保护数据库免受高频聊天请求的冲击，所有聊天记录先写入内存数组，并异步推送至 RabbitMQ，由后台消费者（Worker）平滑写入 MySQL，极大提升系统并发吞吐量。会话被 LRU 淘汰或进程停机时，内存中尚未落库的消息会由 `Flush` 直接回写 DB 兜底，确保上下文不丢且与 MQ 不重复落库（详见 `ARCHITECTURE.md` §13.7）。
...
- **消息队列**: RabbitMQ (异步聊天消息落库，淘汰 / 停机时 Flush 兜底回写)
...
项目采用单仓（Mono-repo）分层架构，各功能模块通过 `common/` 公共基础设施层承载，统一管理，核心能力覆盖：

- **AI 聊天引擎**：流式 SSE 输出、多模型工厂（OpenAI / Ollama）、多会话隔离与上下文记忆，配合 LRU 容量治理控制内存；
- **多模态增强**：图像识别（ONNXRuntime + MobileNetV2 本地推理）、语音合成（TTS）；
- **检索增强（RAG）**：结合向量知识库为大模型提供私有数据增强；
- **协议扩展**：MCP（Model Context Protocol）客户端，支持大模型调用外部工具链；
- **高并发基础设施**：RabbitMQ 异步落库、Redis 缓存、JWT 鉴权、SMTP 邮件。

核心目录说明：

```text
GopherAI/
├── app/                # 应用骨架：依赖装配、启动/优雅停机生命周期管理 (Bootstrap)
├── common/             # 公共基础设施层
...
# 会话缓存容量治理（LRU + 空闲回收）配置示例：
[sessionCacheConfig]
maxSessions = 5000      # 内存中最多保留的会话数，超过则按 LRU 淘汰最久未用
idleTimeoutSec = 1800   # 会话空闲超过该秒数则被后台 sweeper 回收（缺省回退 10000 / 30min）
```
